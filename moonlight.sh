...
case $NUM in
	1)
		...
		if [ -f /home/"$USER"/itimmer.gpg ]
		then	
			echo -e "NOTE: GPG Key Exists - Skipping"
		else		
			wget http://archive.itimmer.nl/itimmer.gpg
			chown "$USER":"$USER" /home/"$USER"/itimmer.gpg
		...
		then	
			...
			rm /home/"$USER"/.emulationstation/es_systems.cfg
		fi
		
		echo -e "Copying Systems Config File"
		cp /etc/emulationstation/es_systems.cfg /home/"$USER"/.emulationstation/es_systems.cfg
			
		if grep -q "<platform>steam</platform>" /home/"$USER"/.emulationstation/es_systems.cfg; then
			echo -e "NOTE: Steam Entry Exists - Skipping"
		else
			echo -e "Adding Steam to Systems"
			sudo sed -i -e 's|</systemList>|  <system>\n    <name>steam</name>\n    <fullname>Steam</fullname>\n    <path>~/RetroPie/roms/moonlight</path>\n    <extension>.sh .SH</extension>\n    <command>bash %ROM%</command>\n    <platform>steam</platform>\n    <theme>steam</theme>\n  </system>\n</systemList>|g' /home/"$USER"/.emulationstation/es_systems.cfg
		fi
		echo -e "\n**** PHASE SIX Complete!!!! ****"
		...
		mkdir -p /home/"$USER"/RetroPie/roms/moonlight
		cd /home/"$USER"/RetroPie/roms/moonlight
		
		echo -e "Create Scripts"
		if [ -f /home/"$USER"/RetroPie/roms/moonlight/720p30fps.sh ]; then
		...
		if [ -f /home/"$USER"/RetroPie/roms/moonlight/720p60fps.sh ]; then
		...
		if [ -f /home/"$USER"/RetroPie/roms/moonlight/1080p30fps.sh ]; then
		...
		if [ -f /home/"$USER"/RetroPie/roms/moonlight/1080p60fps.sh ]; then
		...
		chown -R "$USER":"$USER" /home/"$USER"/RetroPie/roms/moonlight/
		chown "$USER":"$USER" /home/"$USER"/.emulationstation/es_systems.cfg
	...
	4)  
		...
		if [ -f /home/"$USER"/RetroPie/roms/moonlight/480p30fps.sh ]; then
		...
		if [ -f /home/"$USER"/RetroPie/roms/moonlight/480p60fps.sh ]; then
	...
	8) 
		...
		if [ -f /home/"$USER"/moonlight.sh ]
		then	
			echo -e "Removing Script"
			rm /home/"$USER"/moonlight.sh
		fi
		wget https://techwiztime.com/moonlight.sh --no-check
		chown "$USER":"$USER" /home/"$USER"/moonlight.sh
